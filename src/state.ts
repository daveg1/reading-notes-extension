...
class State {
  ...
  get notes() {
    return this.#notes.sort(
      this.#options.isAsc ? ascendingPredicate : descendingPredicate
    )
  }
  ...
  async noteObjectFromUrl(sourceUrl: string): Promise<Note | null> {
    ...
    const sourceTitle =
      (await getPageTitleForUrl(sourceUrl)) ??
      (await getActiveTab()).title ??
      ''
...
const optionsStorage = (await getStoreValue<Options>(OPTIONS_STORAGE_KEY)) ?? {
  isGrouped: false,
  isAsc: true,
}
